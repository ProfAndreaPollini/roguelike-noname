...
RexImage::RexImage(int _width, int _height, int num_layers)
    ...
    for (int i = 0; i < getNumLayers(); ++i) layers[i] = RexLayer(width, height);
...
RexLayer::RexLayer(int width, int height) { tiles.resize(width * height); }

RexLayer::~RexLayer() { tiles.clear(); }
}  // namespace xp